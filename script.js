...
const renderCalendar = (date) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        dayEl.addEventListener('click', (e) => {
            if (e.target.classList.contains('day-card-inner') || e.target.closest('.day-card-inner')) {
                // Sadece kartın arka yüzü boşsa veya Ctrl/Cmd tuşu basılıysa düzenleme modalını aç
                if (food === '' || e.ctrlKey || e.metaKey) {
        ...
        cardBack.addEventListener('click', (e) => {
            e.stopPropagation(); // Üstteki dayEl dinleyicisini engelle
            openEditModal(dateKey, hours || 0, friend, food);
        });
...
// openEditModal fonksiyonu 'food' parametresi alacak şekilde güncellendi
const openEditModal = (dateKey, hours, friend, food) => {
    ...
    document.getElementById('edit-food-list').value = food; // YENİ: Yemek listesi
...
// Tek Gün Kaydetme Formu (GÜNCELLENDİ)
document.getElementById('edit-form').addEventListener('submit', (e) => {
    ...
    const newFood = document.getElementById('edit-food-list').value.trim(); // YENİ: Yemek listesi
...
// --- Toplu Giriş Formu Submit Olayı (GÜNCELLENDİ) ---
fullMonthShiftForm.addEventListener('submit', (e) => {
    ...
    const defaultFoodList = document.getElementById('default-food-list').value.trim(); // YENİ: Varsayılan yemek listesi
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Toplu girişte yemek listesi gün bazında olmadığı için varsayılanı kullanacağız.
        const existingShift = shifts[dateKey] || {};

        if (hoursSelect) {
            const hours = parseInt(hoursSelect.value); 
            let friend = friendInput ? friendInput.value.trim() : ''; 
            let food = existingShift.food || defaultFoodList; // Mevcudu koru, yoksa varsayılanı kullan
            
            if (hours > 0 && friend === '') {
                friend = defaultFriendName;
            }
            // Nöbet kaldırılırsa veya her şey boşsa sil
            if (hours > 0 || food !== '') {
...
// --- Diğer Olay Dinleyicileri ve Fonksiyonlar (Aynı) ---

// Veri Aktarma Fonksiyonları (Aynı)
const exportData = () => {
...
const openFullMonthInputModal = (date) => {
    ...
    document.getElementById('default-food-list').value = ''; // YENİ: Varsayılan yemek listesini temizle
