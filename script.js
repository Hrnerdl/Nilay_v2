...
const saveShifts = () => localStorage.setItem('shifts', JSON.stringify(shifts));
...
const generateDayInputs = (year, month) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const existingMeals = existingShift.meals || '';
        ...
        const mealsInputHtml = `<textarea id="meals-${dateKey}" name="meals-${dateKey}" rows="2" placeholder="Yemekler...">${existingMeals}</textarea>`;
...
fullMonthShiftForm.addEventListener('submit', (e) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        const fullDate = new Date(year, month, day);
        const dateKey = formatDate(fullDate);
        const hours = parseInt(document.getElementById(`hours-${dateKey}`).value); 
        let friend = document.getElementById(`friend-${dateKey}`).value.trim(); 
        let meals = document.getElementById(`meals-${dateKey}`).value.trim();
        if (hours > 0 && friend === '') friend = defaultFriendName;
        if (hours > 0 || meals) {
            shifts[dateKey] = { hours, friend, meals };
        } else {
...
const renderCalendar = (date) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const meals = shiftData ? shiftData.meals : '';
        ...
        if (hours || meals) {
            ...
            if (meals) {
                const mealsInfoEl = document.createElement('div');
                mealsInfoEl.classList.add('meals-info');
...
const openEditModal = (dateKey, hours, friend, meals) => {
    ...
    document.getElementById('edit-meals').value = meals || '';
    editModal.style.display = 'block';
};
const closeEditModal = () => { editModal.style.display = 'none'; currentEditingDate = null; };
...
document.getElementById('edit-form').addEventListener('submit', (e) => {
    ...
    const newMeals = document.getElementById('edit-meals').value.trim();
    if (newHours > 0 || newMeals) {
        shifts[currentEditingDate] = { hours: newHours, friend: newFriend, meals: newMeals };
    } else {
...
document.getElementById('delete-shift-btn').addEventListener('click', () => {
    if (!currentEditingDate) return;
    if (confirm(`${currentEditingDate} tarihindeki kaydı silmek istediğinizden emin misiniz?`)) {
        delete shifts[currentEditingDate];
        saveShifts();
        renderCalendar(currentMonth);
        closeEditModal();
